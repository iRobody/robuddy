...
def downlinkFactory(config):
  ...
  if (config.outputMethod == "outputBluetoothASCII" or
    ...
    downlink = BluetoothDownlinkICreate(
        bluetoothAddress=config.bluetoothAddress)
  elif config.outputMethod == "ADK":
    downlink = ADKDownlink()
  else:
...
# Added for ADK communication with iRobuddy (Arduino robot)
class ADKDownlink(Downlink):

  def __init__(self):
    self.droid = threadedAndroid.LockedAndroid()
    print "Initializing ADK connection"
    
  def WriteCommand(self, msg):

    print "Send msg : %s " % msg
    
    action='com.irobuddy.robot.msg'
    extras = {}
    extras['MSG_TYPE']=msg
    packageName = 'com.irobuddy'
    className = 'Supervisor'
    #intent = self.droid.makeIntent(action, None, None, extras, None, None, None, None).result
    self.droid.startActivity(action, None, None, extras, None, packageName, className)
